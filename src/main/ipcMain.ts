...
ipcMain.handle(IpcEvents.INIT_FILE_WATCHERS, ({ sender }) => {
    ...
    let themesWatcher: FSWatcher | undefined;
    ...
    open(QUICK_CSS_PATH, "a+").then(fd => {
        ...
        try {
            ...
            console.error("[403Cord] QuickCSS watcher başlatılamadı:", e);
        }
    }).catch(() => { });

    try {
        themesWatcher = watch(THEMES_DIR, { persistent: false }, debounce(() => {
            sender.postMessage(IpcEvents.THEME_UPDATE, void 0);
        }));
    } catch (e) {
        console.error("[403Cord] Themes watcher başlatılamadı:", e);
    }
    ...
    if (IS_DEV) {
        try {
            ...
            console.error("[403Cord] Renderer CSS watcher başlatılamadı:", e);
        }
    }
...
ipcMain.handle(IpcEvents.OPEN_MONACO_EDITOR, async () => {
    const title = "403Cord QuickCSS Editor";

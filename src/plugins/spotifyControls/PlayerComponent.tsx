...
function VolumeControl({ volume }: { volume: number; }) {
    ...
    const onChange = React.useMemo(() => debounce((v: number) => {
    ...
    return (
        <Popout
            ...
            renderPopout={() => (
                <div className={cl("volume-popout")}>
                    <VencordSlider
                        initialValue={volume}
                        minValue={0}
                        maxValue={100}
...
function Controls({ volume }: { volume: number; }) {
...
function makeLinkProps(type: "Şarkı" | "Sanatçı" | "Albüm", condition: unknown, name: string, path: string) {
    if (!condition) return {};
        ...
        <div id={cl("info-wrapper")}>
            ...
            <div id={cl("titles")}>
                <Paragraph
                    ...
                    title={track.name}
                    {...makeLinkProps("Şarkı", track.id, track.name, `/track/${track.id}`)}
                >
                    {track.name}
                </Paragraph>
                        ...
                        {track.artists.map((a, i) => (
                            <React.Fragment key={a.name}>
                                <span
                                    ...
                                    title={a.name}
                                    {...makeLinkProps("Sanatçı", a.id, a.name, `/artist/${a.id}`)}
                                >
                                    {a.name}
                                </span>
                ...
                {track.album.name && (
                    <Paragraph className={cl(["ellipoverflow", "secondary-song-info"])}>
                        ...
                        <span
                            ...
                            {...makeLinkProps("Albüm", track.album.id, track.album.name, `/album/${track.album.id}`)}
                        >
                            {track.album.name}
                        </span>
                    </Paragraph>
...
export function Player() {
    ...
    const [isPlaying, volume] = useStateFromStores(
        [SpotifyStore],
        () => [SpotifyStore.isPlaying, SpotifyStore.volume]
    );
    ...
    return (
        <div id={cl("player")} className={cl("compact")} style={exportTrackImageStyle}>
